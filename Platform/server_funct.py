...
def fedlaw_optimization(args, size_weights, parameters, central_node):
    ...
    if args.whether_swa == 'none':
        ...
        if args.server_funct == 'exp':
            optimizees = torch.tensor([torch.log(torch.tensor(j)) for j in size_weights] + [0.0], device='cuda',
                                      requires_grad=True)
        elif args.server_funct == 'quad':
            optimizees = torch.tensor([math.sqrt(1.0 / cohort_size) for j in size_weights] + [1.0], device='cuda',
                                      requires_grad=True)
        ...
        if args.server_funct == 'exp':
            ...
            optmized_weights = [j * j / sum(swa_model.module * swa_model.module).detach().cpu().numpy() for j in
                                swa_model.module.detach().cpu().numpy()]
            ...
            for _, (data, target) in enumerate(train_loader):
                ...
                for i in range(cohort_size):
                    if i == 0:
                        ...
                        if args.server_funct == 'exp':
                            model_param = model_param.add(
                                torch.exp(optimizees[-1]) * optmized_weights[i] * parameters[i])
                        elif args.server_funct == 'quad':
                            model_param = model_param.add(
                                optimizees[-1] * optimizees[-1] * optmized_weights[i] * parameters[i])
...
def fedadam(args, central_node, avg_global_param):
    ...
    # compute param
    for delta_w_t_param, m_param, v_param, w_t_param, w_param in zip(delta_w_t.parameters(),
                                                                     central_node.m.parameters(),
                                                                     central_node.v.parameters(), w_t.parameters(),
                                                                     w.parameters()):
