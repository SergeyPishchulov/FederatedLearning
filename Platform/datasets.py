        ...
        if args.dataset == 'cifar10':
            ...
            if args.iid == 0:  # noniid
                ...
                if args.dirichlet_alpha2:
                    ...
                    groups, proportion = build_non_iid_by_dirichlet_LT(random_state=random_state,
                                                                       dataset=self.train_set,
                                                                       lt_rho=args.longtail_clients,
...
class DataLoader(object):
    ...
    def __init__(self, node_num, dataset, federated_task_config: FederatedMLTaskConfiguration):
        ...
        if dataset == 'cifar10':
            ...
            if federated_task_config.iid == 0:  # noniid
                random_state = np.random.RandomState(int(args.random_seed))
                num_indices = len(self.train_set)
                raise NotImplemented("Need dirichlet in FederatedMLTaskConfiguration")
                if args.dirichlet_alpha2:
                    groups, proportion = build_non_iid_by_dirichlet_hybrid(random_state=random_state,
                                                                           dataset=self.train_set,
                                                                           non_iid_alpha1=args.dirichlet_alpha,
